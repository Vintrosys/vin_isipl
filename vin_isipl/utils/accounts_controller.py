...
from erpnext.controllers.accounts_controller import add_taxes_from_tax_template, set_child_tax_template_and_map, set_order_defaults, validate_and_delete_children
import frappe
...
def update_child_qty_rate(parent_doctype, trans_items, parent_doctype_name, child_docname="items"):
	...
	for d in data:
		...
		if not d.get("docname"):
			...
			prev_disc, new_disc = child_item.get("discount_percentage"), d.get("discount_percentage")
			prev_item_tax, new_item_tax = child_item.get("item_tax_template"), d.get("item_tax_template")
			prev_price_list_rate, new_price_list_rate = child_item.get("price_list_rate"), d.get("price_list_rate")
			...
			disc_unchanged = prev_disc == new_disc
			item_tax_unchanged = prev_item_tax == new_item_tax
		...
		if parent_doctype == "Purchase Order":
			...
			if d.get("price_list_rate"):
				child_item.price_list_rate = d.get("price_list_rate")

			if "discount_percentage" in d:
				child_item.discount_percentage = d.get("discount_percentage")
				
			if d.get("item_tax_template"):
				child_item.item_tax_template = d.get("item_tax_template")
		...
		if flt(child_item.price_list_rate):
			...
			child_item.rate = flt(
				child_item.price_list_rate * (1 - flt(child_item.discount_percentage) / 100.0),
				child_item.precision("rate")
			)
