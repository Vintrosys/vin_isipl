...
class ReceivablePayableReport:
	...
	def get_data(self):
		...
		self.get_sales_invoices_or_customers_based_on_sales_type()
		self.get_sales_invoices_or_customers_based_on_sales_category()
	...
	def get_invoices(self, ple):
		if ple.voucher_type in ("Sales Invoice", "Purchase Invoice"):
			...
			if not (self.filters.get("custom_sales_person") or self.filters.get("custom_invoice_type") or self.filters.get("custom_sales_category")):
			...
			if self.filters.get("custom_sales_person"):
				match = match and (
					ple.voucher_no in self.sales_person_records.get("Sales Invoice", []) or 
					ple.party in self.sales_person_records.get("Customer", [])
				)

			if self.filters.get("custom_invoice_type"):
				match = match and (
					ple.voucher_no in self.sales_type_records.get("Sales Invoice", []) or 
					ple.party in self.sales_type_records.get("Customer", [])
				)

			if self.filters.get("custom_sales_category"):
				match = match and (
					ple.voucher_no in self.sales_category_records.get("Sales Invoice", []) or 
					ple.party in self.sales_category_records.get("Customer", [])
				)
	...
	def get_voucher_balance(self, ple):
		...
		# Check for Sales Type filter
		if self.filters.get("custom_invoice_type"):
			if not (
				ple.party in self.sales_type_records.get("Customer", [])
				or ple.against_voucher_no in self.sales_type_records.get("Sales Invoice", [])
			):
				return

		# Check for Sales Category filter
		if self.filters.get("custom_sales_category"):
			if not (
				ple.party in self.sales_category_records.get("Customer", [])
				or ple.against_voucher_no in self.sales_category_records.get("Sales Invoice", [])
			):
				return
	...
	def get_invoice_details(self):
		...
		if self.account_type == "Receivable":
			...
			si_list = frappe.db.sql(
				"""
				select name, due_date, po_no, custom_sales_person, custom_invoice_type, custom_sales_category
		...
		if self.account_type == "Payable":
			...
			for pi in frappe.db.sql(
				"""
				select name, due_date, bill_no, bill_date, custom_sales_person
	...
	def get_sales_invoices_or_customers_based_on_sales_person(self):
		if self.filters.get("custom_sales_person"):
			...
			# Get Sales Invoices linked with selected Sales Person
			...
			for invoice in sales_invoices:
				self.sales_person_records.setdefault("Sales Invoice", set()).add(invoice)
			...
			for customer in customers:
				self.sales_person_records.setdefault("Customer", set()).add(customer)

	def get_sales_invoices_or_customers_based_on_sales_type(self):
		if self.filters.get("custom_invoice_type"):
			sales_types = self.filters.get("custom_invoice_type")  

			self.sales_type_records = frappe._dict()

			# Get Sales Invoices linked with selected Sales Type
			sales_invoices = frappe.get_all(
				"Sales Invoice",
				filters={
					"custom_invoice_type": ["in", sales_types],
					"docstatus": 1
				},
				pluck="name"
			)
			...
			for invoice in sales_invoices:
				self.sales_type_records.setdefault("Sales Invoice", set()).add(invoice)

			
	def get_sales_invoices_or_customers_based_on_sales_category(self):
		if self.filters.get("custom_sales_category"):
			sales_category_list = self.filters.get("custom_sales_category")  

			self.sales_category_records = frappe._dict()

			# Get Sales Invoices linked with selected Sales Category
			sales_invoices = frappe.get_all(
				"Sales Invoice",
				filters={
					"custom_sales_category": ["in", sales_category_list],
					"docstatus": 1
				},
				pluck="name"
			)
			...
			for invoice in sales_invoices:
				self.sales_category_records.setdefault("Sales Invoice", set()).add(invoice)
